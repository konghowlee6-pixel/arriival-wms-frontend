...
interface ResetPasswordProps {
  token: string;
  onResetSuccess: (session: { user: User; org: Organization | null }) => void;
}
...
const ResetPassword: React.FC<ResetPasswordProps> = ({ token, onResetSuccess }) => {
  ...
  useEffect(() => {
    ...
    const userToReset = allUsers.find(u => u.passwordResetToken === token);
    
    if (userToReset && userToReset.passwordResetExpires && userToReset.passwordResetExpires > Date.now()) {
      setUser(userToReset);
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!user) {
        setError('User not found. Cannot reset password.');
        return;
    }
    ...
    if (!updatedUser) {
        setError('Failed to update password. Please contact support.');
        return;
    }

    const allOrgs = JSON.parse(localStorage.getItem('inventory-app-orgs') || '[]') as Organization[];
    const org = allOrgs.find(o => o.id === updatedUser.organizationId);

    alert('Password reset successfully! You will now be logged in.');
    onResetSuccess({ user: updatedUser, org: org || null });
  };
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 space-y-6">
        ...
        {isLoading ? (
            ...
            <>
                <p className="text-center text-gray-500">Enter a new password for <span className="font-semibold">{user.email}</span>.</p>
